...
export class ProductService {
  ...
  private async findOrCreateCategoryByName(name: string): Promise<Category> {
    ...
    let entity = await this.categoryRepository
      ...
      .getOne();
    if (!entity) {
      entity = this.categoryRepository.create({
        name: trimmed,
        description: null,
  ...
  private async findOrCreateBrandByName(name: string): Promise<Brand> {
    ...
    let entity = await this.brandRepository
      ...
      .getOne();
    if (!entity) {
      entity = this.brandRepository.create({
        name: trimmed,
        description: null,
        logoUrl: null,
      });
  ...
  async update(id: string, payload: ProductPayload) {
    ...
    if (payload.frameStyle !== undefined)
      existing.frameStyle = payload.frameStyle;
    if (payload.frameColor !== undefined)
      existing.frameColor = payload.frameColor;
    ...
    if (payload.frameWidth !== undefined)
      existing.frameWidth = payload.frameWidth;
    ...
    if (payload.lensImageUrl !== undefined)
      existing.lensImageUrl = payload.lensImageUrl ?? null;
    ...
    if (payload.brandId !== undefined) {
      if (payload.brandId) {
        const br = await this.brandRepository.findOneBy({
          id: payload.brandId,
        });
        if (!br)
          throw new NotFoundException(`Brand ${payload.brandId} not found`);
        existing.brand = br;
      } else {
        existing.brand = null;
      }
  ...
  private applyListFilters(
    ...
    if (query.brandId) {
      ...
      qb.andWhere('LOWER(brand.name) = LOWER(:brandName)', {
        brandName: query.brand.trim(),
      });
    }
    if (query.frameStyle?.trim())
      qb.andWhere('p.frameStyle = :frameStyle', {
        frameStyle: query.frameStyle.trim(),
      });
    if (query.frameColor?.trim())
      qb.andWhere('p.frameColor = :frameColor', {
        frameColor: query.frameColor.trim(),
      });
    if (query.shape?.trim())
      qb.andWhere('p.shape = :shape', { shape: query.shape.trim() });
    if (query.material?.trim())
      qb.andWhere('p.material = :material', {
        material: query.material.trim(),
      });
    if (query.frameWidth)
      qb.andWhere('p.frameWidth = :frameWidth', {
        frameWidth: query.frameWidth,
      });
    ...
    if (query.search?.trim()) {
      ...
      qb.andWhere(
        new Brackets((sub) => {
          sub
            .where('brand.name ILIKE :term', { term })
  ...
  private async distinctTrimmedColumn(
    ...
    const rows = await this.productRepository
      ...
      .getRawMany();
    return rows
      .map((r: { value: string | null }) => String(r.value ?? '').trim())
      .filter((v) => v.length > 0);
  }
